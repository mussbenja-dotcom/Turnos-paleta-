...
const { reservarManual, reservarRecurrente } = require('../services/reservaService');
...
<div class="container">
  ...
  <div class="controls">
    ...
    <button onclick="generarTurnos()">
      Generar turnos (30 días)
    </button>
...
<div class="modal-fondo" id="modalReserva">
  <div class="modal">
    ...
    <div class="acciones">
      <button class="cancelar" onclick="cerrarReserva()">
        Cancelar
      </button>
...
async function generarTurnos() {
  ...
  if (!confirm('¿Generar los turnos de los próximos 30 días?')) {
    return;
  }
...
async function confirmarReserva() {
  const nombre =
    document.getElementById('modalNombre').value.trim();

  const recurrente =
    document.getElementById('modalRecurrente').checked;
  ...
  try {
    const respuesta =
      await fetch('/admin/api/reservar', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
