...
router.post('/webhook', async (req, res) => {
  ...
  try {
    ...
    if (reply?.id?.startsWith('dia_')) {
      ...
      const turnos = await disponiblesPorFecha({ tipo, fecha });

      if (turnos.length) {
        await enviarListaTurnos(from, turnos, 'Turnos disponibles');
      } else {
        await enviarTexto(from, 'No quedan turnos disponibles para ese día 😕');
      }
      return;
    }
    ...
    // =================================================================
    // 6. CUALQUIER OTRO MENSAJE → MENÚ PRINCIPAL
    // =================================================================
