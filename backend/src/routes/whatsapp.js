...
// Arma la línea de precio para el saludo (vacía si el precio es 0).
async function lineaPrecio() {
  const config = await obtenerConfig();
  if (config.precio > 0) {
    return `\n💲 Valor del turno: *${fmtPrecio(config.precio)}*\n`;
  }
  return '';
}
...
router.post('/webhook', async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      const precioTxt1 = await lineaPrecio();
      await enviarTexto(from, 'Empecemos de nuevo 🔄' + precioTxt1 + '\nReservá tu cancha de pádel 👇');
    ...
    if (reply?.id?.startsWith('pago_')) {
      ...
      // Registramos el medio de pago elegido, pero la reserva queda PENDIENTE.
      // El pago lo confirma el administrador desde el panel.
      const medioBd = medio === 'transferencia' ? 'transferencia' : 'efectivo';
      await registrarPago(reserva.id, { medioPago: medioBd, monto: null, pagado: false }).catch(() => {});
    ...
    // =================================================================
    {
      ...
      const precioTxt2 = await lineaPrecio();
      await enviarTexto(from, '¡Hola! 🎾 Bienvenido al Club.' + precioTxt2 + '\nReservá tu cancha de pádel 👇');
      await enviarListaDias(from, dias, 'padel');
    }
