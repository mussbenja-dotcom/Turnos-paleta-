...
// Alias para transferencias (configurable en Railway con la variable ALIAS_PAGO).
const ALIAS_PAGO = process.env.ALIAS_PAGO || 'club.padel.mp';
...
router.post('/webhook', async (req, res) => {
  ...
  try {
    ...
    if (reply?.id?.startsWith('franja_')) {
      ...
      if (turnos.length) {
        const titulo = franja === 'manana' ? 'Turnos de la mañana' : 'Turnos de la tarde/noche';
        await enviarListaTurnos(from, turnos, titulo);
      } else {
        await enviarTexto(
          from,
          franja === 'manana'
            ? 'No quedan turnos disponibles a la mañana para ese día 😕\nProbá con *Tarde/Noche* o elegí otro día escribiendo *hola*.'
            : 'No quedan turnos disponibles a la tarde/noche para ese día 😕\nProbá con *Mañana* o elegí otro día escribiendo *hola*.'
        );
      }
      return;
    }
