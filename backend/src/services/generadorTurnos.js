 ...
 *
 * Las horas se construyen directamente con el offset de Argentina, en vez de
 * usar setHours() (que dependía de la zona horaria del servidor, UTC en Railway,
 * y corría los turnos varias horas).
 */
async function generarTurnosDelDia(canchaId, fecha) {
  ...
  // Construimos el instante exacto en hora de Argentina.
  let cursor = new Date(`${fecha}T${pad(ha)}:${pad(ma)}:00${TZ}`);
  const cierre = new Date(`${fecha}T${pad(hc)}:${pad(mc)}:00${TZ}`);
