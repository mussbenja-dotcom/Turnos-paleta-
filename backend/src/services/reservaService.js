 ...
 *
 * @param {boolean} pagado  Si es true, marca la reserva como PAGADA.
 *                          Si es false, solo guarda el medio de pago elegido
 *                          pero la reserva queda PENDIENTE (la confirma el admin).
 *                          Por defecto true, para no romper llamadas existentes.
 */
async function registrarPago(reservaId, { medioPago, monto, pagado = true }) {
  const { rows } = await pool.query(
    `UPDATE reservas SET pagado=$4, medio_pago=$2, monto=COALESCE($3, monto)
     WHERE id=$1 RETURNING *`,
    [reservaId, medioPago, monto, pagado]
  );
  return rows[0] || null;
}
