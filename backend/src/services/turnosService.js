...
async function disponiblesPorFecha({ tipo, fecha, limit = 10, franja = null }) {
  // franja: 'manana' => 07:00-11:59 | 'tarde' => 12:00-17:59 | 'noche' => 18:00+ | null => todo el día
  let filtroFranja = '';
  if (franja === 'manana') {
    filtroFranja = ` AND EXTRACT(HOUR FROM (t.inicio AT TIME ZONE '${TZ}')) < 12`;
  } else if (franja === 'tarde') {
    filtroFranja = ` AND EXTRACT(HOUR FROM (t.inicio AT TIME ZONE '${TZ}')) >= 12
                     AND EXTRACT(HOUR FROM (t.inicio AT TIME ZONE '${TZ}')) < 18`;
  } else if (franja === 'noche') {
    filtroFranja = ` AND EXTRACT(HOUR FROM (t.inicio AT TIME ZONE '${TZ}')) >= 18`;
  }
  const { rows } = await pool.query(
    `SELECT t.id, t.inicio, t.fin, c.nombre AS cancha_nombre, c.tipo
     FROM turnos t
